...
const workflowStageSchema = z.enum(["WAITING", "IN_WORK", "REVIEW", "DONE"])

export async function updateUnifiedRecordWorkflowStatusAction(id: string, stage: z.infer<typeof workflowStageSchema>) {
    const session = await auth()
    if (!session?.user?.email) throw new Error("Unauthorized")
    ...
    if (user.role !== "ADMIN" && !user.permProcessUnifiedRecords && !user.permAssignUnifiedRecords && !user.permManageUnifiedRecords) {
        throw new Error("У вас немає прав для зміни етапу")
    }
    ...
    if (!record) throw new Error("Запис не знайдено")
    if (record.recordType !== "EO" && record.recordType !== "ZVERN") {
        throw new Error("Kanban доступний тільки для ЄО та Звернень")
    }

    if (user.role !== "ADMIN" && record.assignedUserId && record.assignedUserId !== user.id && !user.permAssignUnifiedRecords) {
        throw new Error("Можна змінювати тільки власні призначені записи")
    }

    const now = new Date()
    const updateData: any = {}

    if (parsedStage === "WAITING") {
        updateData.status = "PENDING"
        updateData.processedAt = null
        updateData.resolutionDate = null
    } else if (parsedStage === "IN_WORK") {
        updateData.status = "IN_PROGRESS"
        updateData.processedAt = null
        updateData.resolutionDate = null
    } else if (parsedStage === "REVIEW") {
        updateData.status = "REVIEW"
        updateData.processedAt = null
        updateData.resolutionDate = null
        if (!record.resolution || !record.resolution.trim()) {
            updateData.resolution = "Передано на перевірку"
        }
    } else if (parsedStage === "DONE") {
        updateData.status = "PROCESSED"
        updateData.processedAt = now
        updateData.resolutionDate = now
        if (!record.resolution || !record.resolution.trim()) {
            updateData.resolution = "Завершено"
        }
    }

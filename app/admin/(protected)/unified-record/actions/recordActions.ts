...
const UnifiedRecordSchema = z.object({
    ...
    recordType: z.enum(["EO", "ZVERN", "DETENTION"]).default("EO"),
...
async function generateNextDetentionRaportNumber() {
...
export async function getUnifiedRecords(params?: {
    ...
    const where: any = {
        recordType: { in: ["EO", "ZVERN", "DETENTION"] }
    }
...
export async function upsertUnifiedRecordAction(data: any) {
    ...
    const payload = { ...data }
    const isDetention = payload.recordType === "DETENTION"
    const isCreate = !payload.id

    if (isDetention && isCreate) {
        payload.eoNumber = await generateNextDetentionRaportNumber()
    }

    if (isDetention && !isCreate && (!payload.eoNumber || !String(payload.eoNumber).trim()) && payload.id) {
        const existingById = await prisma.unifiedRecord.findUnique({
            where: { id: payload.id },
            select: { eoNumber: true }
        })
        payload.eoNumber = existingById?.eoNumber || await generateNextDetentionRaportNumber()
    }
